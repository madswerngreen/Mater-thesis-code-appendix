...
fuel["geometry"] = fuel.geometry.buffer(10)
...
highways_file = 'C:/Users/b306630/OneDrive - Vejdirektoratet/Master Thesis/Data/Geospatial_info/highways_shape.gpkg'
...
"""
CHAIN_MAX_GAP_SEC = 2 * 3600
CHAIN_MAX_DIST_M  = 250

FERRY_MAX_GAP_SEC = 5 * 3600
...
def read_truck(truck_id, speed_from_file=True):
    df = pd.read_parquet(f"C:/Users/b306630/OneDrive - Vejdirektoratet/Master Thesis/Data/parquet_trucks/truck_{truck_id}.parquet")
...
def postprocess_and_chain(df_moving, truck_id):
    ...
    for i in range(1, len(ordered)):
        ...
        if end_ferry_map.get(prev_tid, None) == expected_arrival_port:
            gap = (trip_stats_local.loc[curr_tid, "start_time"] -
                   trip_stats_local.loc[prev_tid, "end_time"]).total_seconds()
            if gap <= FERRY_MAX_GAP_SEC:
                merged_id[curr_tid] = merged_id[prev_tid]
                continue
    
        # ----------------------------------------------
        # Ruleif gap <= CHAIN_MAX_GAP_SEC: 2: Two-trip ferry:
        ...
        if i >= 2:
            ...
            if end_ferry_map.get(prevprev_tid, None) == expected_arrival_port:
                gap = (trip_stats_local.loc[curr_tid, "start_time"] -
                       trip_stats_local.loc[prevprev_tid, "end_time"]).total_seconds()
                if gap <= FERRY_MAX_GAP_SEC:
...
def get_trips_for_config(truck_id, cfg_name, cfg_params):
    try:
        ...
        if len(df_final) > 0:
            outdir = os.path.join(OUTPUT_ROOT, cfg_name)
            os.makedirs(outdir, exist_ok=True)
            df_final.to_parquet(f"{outdir}/truck_{truck_id}_trips.parquet")
        else:
            print(f"{cfg_name} | Truck {truck_id}: no valid trips")
...
if __name__ == "__main__":
    files = glob.glob("C:/Users/b306630\Desktop/Master Thesis/Data/parquet_trucks/truck_*.parquet")
    truck_ids = sorted(int(f.split("_")[-1].split(".")[0]) for f in files)

    for cfg_name, cfg_params in CONFIGS.items():
        print(f"\n=== Running CONFIG: {cfg_name} ===\n")

        with tqdm_joblib(tqdm(total=len(truck_ids), desc=f"{cfg_name}")):
            Parallel(n_jobs=8, backend="loky")(
                delayed(get_trips_for_config)(i, cfg_name, cfg_params)
                for i in truck_ids
            )
